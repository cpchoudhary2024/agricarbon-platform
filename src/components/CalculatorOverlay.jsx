    ...
    <div ref={ref} style={BASE_OVERLAY}>
      ...
      <div style={{ width: '100%', maxWidth: '1080px', paddingBottom: '28px', pointerEvents: 'auto', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(254,253,249,0.99) 0%, rgba(247,242,232,0.98) 100%)',
        }}>
          <p className="eyebrow-premium">Bedrock Lab · IPCC Tier 1 Carbon Calculator</p>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              ...
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 'clamp(12px,1.2vw,14px)', color: F.muted, margin: 0, maxWidth: '520px', lineHeight: 1.65 }}>
                Compare <em>what you do now</em> (Baseline) against a <em>planned intervention</em> (Scenario) to find the true carbon additionality of any practice change.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[['SOC', 'Soil Organic Carbon'], ['CO₂e', 'Equiv. Emissions'], ['±CI', '95% Confidence']].map(([v, l]) => (
                <div key={v} style={{
                  padding: '8px 14px', background: G['100'],
                  border: `1px solid ${G['300']}40`, borderRadius: '10px', textAlign: 'center',
                }}>
                  <div style={{ fontFamily: 'Inter, sans-serif', fontVariantNumeric: 'tabular-nums', fontSize: '13px', fontWeight: 600, color: G['700'] }}>{v}</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: '12px', marginBottom: '14px' }}>
          ...
          <div style={{ ...PANEL, borderTop: `2px solid ${Au['500']}` }}>
            <Lbl>Carbon Price (USD / t CO₂e)</Lbl>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '10px', color: F.faint, margin: '6px 0 0', lineHeight: 1.5 }}>
              Voluntary market avg: $20–60/t. Leave blank to omit.
            </p>
          </div>
        </div>

        {/* ── Carbon Price Tracker (market reference panel) ── */}
        <CarbonPriceTracker onSelectPrice={p => setPrice(String(p))} />

        {/* ── ROI Full Financial Model link ── */}
        <div style={{
          ...PANEL,
          marginBottom: '14px',
          borderLeft: `4px solid ${G['700']}`,
          padding: '18px 20px',
          display: 'flex', gap: '14px', alignItems: 'center', flexWrap: 'wrap',
          background: `linear-gradient(135deg, rgba(254,253,249,0.99), rgba(240,253,244,0.96))`,
        }}>
          <div style={{ flex: 1 }}>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '11px', fontWeight: 700, color: G['500'], letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 4px' }}>
              Full Financial Model — New Page
            </p>
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: F.muted, margin: 0, lineHeight: 1.65 }}>
              Go deeper with <strong style={{ color: F.body }}>NPV, IRR, payback analysis</strong>, itemized implementation costs, and government subsidy eligibility (USDA EQIP, CSP + state programs).
            </p>
          </div>
          <Link
            to={`/roi?area=${area}&rate=${result ? (result.co2ePerHaPerYear / 3.667).toFixed(3) : '0.8'}&price=${price || '20'}&years=${years}`}
            style={{
              display: 'inline-flex', alignItems: 'center', gap: '6px',
              padding: '11px 20px',
              background: G['700'], color: '#FEFDF9',
              border: 'none', borderRadius: '10px',
              fontFamily: 'Inter, sans-serif', fontSize: '12px', fontWeight: 700,
              textDecoration: 'none', letterSpacing: '-0.01em',
              boxShadow: `0 4px 16px ${G['700']}30`,
              flexShrink: 0,
            }}
          >
            Open ROI Calculator →
        ...
        {/* ── Best upgrade hint (pre-calculation) ── */}
        {!result && (
          <div style={{ ...PANEL, padding: '16px 20px', marginBottom: '14px', display: 'flex', gap: '14px', alignItems: 'center', borderLeft: `3px solid ${Au['500']}` }}>
            ...
            <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: F.muted, margin: 0, lineHeight: 1.65 }}>
              <strong style={{ color: Au['700'] }}>Tip:</strong> Set your current practice as Baseline, then change <em>one thing</em> in Scenario to isolate the carbon impact of that specific intervention.
            </p>
          </div>
        )}
        ...
        <div style={{ ...PANEL, marginBottom: '14px', borderTop: `3px solid ${healthScenario.grade.color}` }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center' }}>
            ...
            <div style={{
              ...
              background: `conic-gradient(${healthScenario.grade.color} ${healthScenario.score * 3.6}deg, #EDE7DA 0deg)`,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              transition: 'background 0.6s ease',
              boxShadow: `0 4px 24px ${healthScenario.grade.color}30`,
            }}>
              <div style={{
                width: '92px', height: '92px', borderRadius: '50%', background: '#FEFDF9',
                display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
              }}>
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '36px', fontWeight: 700, color: F.head, lineHeight: 1 }}>
                  {healthScenario.score}
                </div>
                <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '8px', color: F.faint, letterSpacing: '0.1em', marginTop: '2px' }}>SCORE / 100</div>
              </div>
            </div>

            <div style={{ flex: '1 1 240px' }}>
              <p className="eyebrow-premium" style={{ marginBottom: '6px' }}>Soil Health Score Card · live</p>
              <h3 style={{
                fontFamily: 'Cormorant Garamond, serif',
                fontSize: 'clamp(18px, 2.2vw, 28px)', fontWeight: 700, color: F.head, margin: '0 0 6px',
                letterSpacing: '-0.02em',
              }}>
                Grade {healthScenario.grade.letter} —{' '}
                <span style={{ color: healthScenario.grade.color }}>{healthScenario.grade.label}</span>
              </h3>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: F.muted, margin: '0 0 12px', lineHeight: 1.65 }}>
                Composite of tillage, organic input, and crop choice from your <em>Scenario</em> panel. Updates instantly as you change inputs.
              </p>
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', fontFamily: 'Inter, sans-serif', fontSize: '11px', color: F.muted }}>
                <span>Baseline grade: <b style={{ color: healthBaseline.grade.color }}>{healthBaseline.grade.letter}</b> ({healthBaseline.score})</span>
                <span style={{ color: F.faint }}>·</span>
                <span>Δ: <b style={{ color: healthScenario.score >= healthBaseline.score ? G['700'] : T['500'] }}>
                  {healthScenario.score >= healthBaseline.score ? '+' : ''}{healthScenario.score - healthBaseline.score} points
                </b></span>
              </div>
            </div>
            ...
            <div style={{ flex: '1 1 220px', minWidth: '200px' }}>
              {[
                ['Tillage', healthScenario.breakdown.tillage],
                ['Organic Inputs', healthScenario.breakdown.inputs],
                ['Crop / Land Use', healthScenario.breakdown.crop],
              ].map(([lbl, b]) => (
                <div key={lbl} style={{ marginBottom: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'Inter, sans-serif', fontSize: '11px', color: F.body, marginBottom: '4px' }}>
                    <span>{lbl} <span style={{ color: F.faint, fontSize: '10px' }}>· {Math.round(b.weight * 100)}%</span></span>
                    <span style={{ fontWeight: 700, color: F.head }}>{b.score}</span>
                  </div>
                  <div style={{ height: '5px', background: '#EDE7DA', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${b.score}%`, background: healthScenario.grade.color, borderRadius: '3px', transition: 'width 0.5s ease' }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ ...PANEL, marginBottom: '14px', borderLeft: `3px solid ${G['500']}` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px', marginBottom: showLibrary ? '14px' : '0' }}>
            <div>
              <p className="eyebrow-premium" style={{ marginBottom: '4px' }}>
                Scenario Library {scenarios.length > 0 && <span style={{ color: F.faint, fontWeight: 400 }}>· {scenarios.length} saved</span>}
              </p>
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: F.muted, margin: 0, lineHeight: 1.5 }}>
                Save, compare and share named scenarios. Stored locally in your browser.
              </p>
            </div>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              <input
                type="text" placeholder="Name this scenario"
                value={saveName} onChange={e => setSaveName(e.target.value)}
                onKeyDown={e => { if (e.key === 'Enter') saveCurrentScenario(); }}
                style={{ ...SEL, width: '180px', padding: '8px 12px', fontSize: '12px' }}
              />
              <button onClick={saveCurrentScenario} className="btn-forest" style={{ padding: '8px 16px', fontSize: '12px' }}>
                + Save
              </button>
              <button onClick={() => setShowLibrary(v => !v)} className="btn-outline-terra" style={{ padding: '8px 16px', fontSize: '12px' }}>
                {showLibrary ? 'Hide' : 'Show'} ({scenarios.length})
              </button>
              <button onClick={copyShareLink} className="btn-outline-terra" style={{ padding: '8px 16px', fontSize: '12px' }}>
          ...
          {showLibrary && (
            scenarios.length === 0 ? (
              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: F.faint, margin: '8px 0 0', fontStyle: 'italic' }}>
                No saved scenarios yet. Configure your inputs and click Save to build a comparison library.
              </p>
            ) : (
              <div style={{ overflowX: 'auto', borderRadius: '10px', border: '1px solid #DDD5C4', maxHeight: '260px', overflowY: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter, sans-serif', fontSize: '11px' }}>
                  <thead style={{ background: '#F7F2E8', position: 'sticky', top: 0 }}>
                    <tr>
                      {['Name', 'Climate', 'Area', 'Yrs', 'Scenario', 'CO₂e (t)', 'ΔSOC', 'Health', ''].map(h => (
                        <th key={h} style={{ padding: '8px 10px', textAlign: 'left', fontSize: '10px', fontWeight: 700, color: F.muted, letterSpacing: '0.06em', textTransform: 'uppercase', borderBottom: '1px solid #DDD5C4', whiteSpace: 'nowrap' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {scenarios.map((s, i) => (
                      <tr key={s.id} style={{ background: i % 2 === 0 ? '#FEFDF9' : 'transparent' }}>
                        ...
                        <td style={{ padding: '7px 10px', color: s.co2eTotal >= 0 ? G['700'] : T['700'], fontWeight: 700 }}>
                          {s.co2eTotal != null ? `${s.co2eTotal >= 0 ? '+' : ''}${s.co2eTotal.toLocaleString(undefined, { maximumFractionDigits: 1 })}` : '—'}
                        </td>
                        <td style={{ padding: '7px 10px', color: F.muted }}>
                          {s.deltaSocPerHa != null ? `${s.deltaSocPerHa >= 0 ? '+' : ''}${s.deltaSocPerHa.toFixed(3)}` : '—'}
                        </td>
        ...
        {/* ── Calculate button ── */}
        <button onClick={calculate} disabled={busy} className="btn-forest" style={{
          width: '100%', marginBottom: '20px', padding: '16px',
          fontSize: '15px', letterSpacing: '-0.01em', justifyContent: 'center',
          opacity: busy ? 0.6 : 1,
        }}>
          {busy
            ? <><span style={{ display: 'inline-block', width: '16px', height: '16px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} /> Calculating…</>
            : 'Calculate Carbon Uplift →'
          }
        </button>
